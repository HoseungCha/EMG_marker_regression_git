...
%     path_emg = fullfile(cd,'DB_v2','emg_win',sprintf('comb_%d',i_comb));
    ...
    path_emg = fullfile(cd,'DB_v2','emg_win_10Hz_Time Alignment','comb_1');
    ...
    for i_sub = 2 : N_sub
        for i_trl = 1 : N_trl
            ...
            len_win = trg_w(27); % 잘라야할 윈도우 싸이즈 얻기
...
%             filter_order = 2;
%             cutoff_Freq = [0.5 ];
%             [B,A] = butter(filter_order, cutoff_Freq/Fn,'low');
%             filted_dif_mark_n = filtfilt(B,A,diff(mark_n));
% 
%             plot(filted_dif_mark_n)
%             
